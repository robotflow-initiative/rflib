...
def boxes_iou_bev(boxes_a, boxes_b):
    ...
    ext_module.boxes_iou_bev_gpu(boxes_a.contiguous(), boxes_b.contiguous(),
                                 ans_iou)
...
def nms_gpu(boxes, scores, thresh, pre_maxsize=None, post_max_size=None):
    ...
    num_out = ext_module.nms3d_gpu(boxes, keep, thresh, boxes.device.index)
...
def nms_normal_gpu(boxes, scores, thresh):
    ...
    keep = torch.zeros(boxes.size(0), dtype=torch.long)
    num_out = ext_module.nms3d_normal_gpu(boxes, keep, thresh,
                                        boxes.device.index)
