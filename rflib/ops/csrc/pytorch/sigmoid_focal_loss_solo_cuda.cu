...
__global__ void SigmoidFocalLossSOLOForward(const int nthreads,
...
}  // SigmoidFocalLossSOLOForward

template <typename scalar_t>
__global__ void SigmoidFocalLossSOLOBackward(
...
}  // SigmoidFocalLossSOLOBackward

at::Tensor SigmoidFocalLoss_solo_forward_cuda(const at::Tensor &logits,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      logits.scalar_type(), "SigmoidFocalLoss_forward", [&] {
        SigmoidFocalLossSOLOForward<scalar_t><<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(
...
at::Tensor SigmoidFocalLoss_solo_backward_cuda(const at::Tensor &logits,
                                          const at::Tensor &targets,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      logits.scalar_type(), "SigmoidFocalLoss_backward", [&] {
        SigmoidFocalLossSOLOBackward<scalar_t><<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(
