...
struct RFLIBRoiAlignKernel {
 public:
  RFLIBRoiAlignKernel(Ort::CustomOpApi ort, const OrtKernelInfo* info)
      : ort_(ort) {
...
struct RFLIBRoiAlignCustomOp
    : Ort::CustomOpBase<RFLIBRoiAlignCustomOp, RFLIBRoiAlignKernel> {
  void* CreateKernel(Ort::CustomOpApi api, const OrtKernelInfo* info) {
    return new RFLIBRoiAlignKernel(api, info);
  }
  const char* GetName() const { return "RFLIBRoiAlign"; }
