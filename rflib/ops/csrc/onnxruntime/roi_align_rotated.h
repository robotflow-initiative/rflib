...
struct RFLIBRoIAlignRotatedKernel {
 public:
  RFLIBRoIAlignRotatedKernel(Ort::CustomOpApi ort, const OrtKernelInfo* info)
      : ort_(ort) {
    aligned_height_ =
...
struct RFLIBRoIAlignRotatedCustomOp
    : Ort::CustomOpBase<RFLIBRoIAlignRotatedCustomOp,
                        RFLIBRoIAlignRotatedKernel> {
  void* CreateKernel(Ort::CustomOpApi api, const OrtKernelInfo* info) {
    return new RFLIBRoIAlignRotatedKernel(api, info);
  }
  const char* GetName() const { return "RFLIBRoIAlignRotated"; }
