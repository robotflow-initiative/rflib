...
size_t get_onnxnms_workspace_size(size_t num_batches, size_t spatial_dimension,
  ...
  if (center_point_box == 1) {
    boxes_xyxy_workspace = rflib::getAlignedSize(
        num_batches * spatial_dimension * 4 * boxes_word_size);
  }
  size_t scores_workspace =
      rflib::getAlignedSize(spatial_dimension * boxes_word_size);
  size_t boxes_workspace =
      rflib::getAlignedSize(spatial_dimension * 4 * boxes_word_size);
  const int col_blocks = DIVUP(spatial_dimension, threadsPerBlock);
  size_t mask_workspace = rflib::getAlignedSize(spatial_dimension * col_blocks *
                                               sizeof(unsigned long long));
  size_t index_template_workspace =
      rflib::getAlignedSize(spatial_dimension * sizeof(int));
  size_t index_workspace =
      rflib::getAlignedSize(spatial_dimension * sizeof(int));
  size_t count_workspace = rflib::getAlignedSize(sizeof(int));
...
void TRTNMSCUDAKernelLauncher_float(const float* boxes, const float* scores,
  ...
  workspace = static_cast<char*>(workspace) +
              rflib::getAlignedSize(spatial_dimension * 4 * sizeof(float));

  float* boxes_xyxy = nullptr;
  if (center_point_box == 1) {
    boxes_xyxy = (float*)workspace;
    workspace = static_cast<char*>(workspace) +
                rflib::getAlignedSize(num_batches * spatial_dimension * 4 *
                                     sizeof(float));
  ...
  workspace = static_cast<char*>(workspace) +
              rflib::getAlignedSize(spatial_dimension * sizeof(float));

  unsigned long long* dev_mask = (unsigned long long*)workspace;
  workspace = static_cast<char*>(workspace) +
              rflib::getAlignedSize(spatial_dimension * col_blocks *
                                   sizeof(unsigned long long));
  ...
  workspace = static_cast<char*>(workspace) + rflib::getAlignedSize(sizeof(int));
