...
static const char *PLUGIN_NAME{"RFLIBDeformConv2d"};
}  // namespace
...
size_t DeformableConvPluginDynamic::getWorkspaceSize(
  ...
  int sizeof_dtype = rflib::getElementSize(outputs[0].type);
  ...
  size_t col_size =
      rflib::getAlignedSize(nInputPlane * kW * kH * im2col_step * outputHeight *
                           outputWidth * sizeof_dtype);

  size_t out_size = 0;
  if (im2col_step != 1)
    out_size = rflib::getAlignedSize(batch_size * nOutputPlane * outputHeight *
                                    outputWidth * sizeof_dtype);
