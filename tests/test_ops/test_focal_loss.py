...
class Testfocalloss(object):
    ...
    def _test_softmax(self, dtype=torch.float):
        if not torch.cuda.is_available():
            return
        from rflib.ops import softmax_focal_loss
        alpha = 0.25
        gamma = 2.0
    ...
    def _test_sigmoid(self, dtype=torch.float):
        if not torch.cuda.is_available():
            return
        from rflib.ops import sigmoid_focal_loss
        alpha = 0.25
        gamma = 2.0
    ...
    def _test_grad_softmax(self, dtype=torch.float):
        if not torch.cuda.is_available():
            return
        from rflib.ops import SoftmaxFocalLoss
        alpha = 0.25
        gamma = 2.0
        for case in inputs:
            np_x = np.array(case[0])
    ...
    def _test_grad_sigmoid(self, dtype=torch.float):
        if not torch.cuda.is_available():
            return
        from rflib.ops import SigmoidFocalLoss
        alpha = 0.25
        gamma = 2.0
        for case in inputs:
            np_x = np.array(case[0])
