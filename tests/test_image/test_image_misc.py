...
def test_tensor2imgs():
    ...
    with pytest.raises(AssertionError):
        ...
        rflib.tensor2imgs(tensor, mean=(1, ))

    # test std length
    with pytest.raises(AssertionError):
        tensor = torch.randn(2, 3, 5, 5)
        rflib.tensor2imgs(tensor, std=(1, ))
    ...
    outputs = rflib.tensor2imgs(tensor, to_rgb=True)
    ...
    outputs = rflib.tensor2imgs(tensor, to_rgb=False)
    for gt, output in zip(gts, outputs):
        assert_array_equal(gt, output)
